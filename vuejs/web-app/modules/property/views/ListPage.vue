				...
				<div class="container">
					<div class="row q-gutter-y-sm">
						...
						<div class="col col-12 col-md-auto pb-4 md:pl-2">
							...
							></q-btn>
							<q-btn-dropdown
								:label="'Sort By: '+ formInput.sortOption?.text"
								class="text-capitalize ml-2"
								outline
								style="height: 40px;"
							>
								<q-list>
									<template v-for="(sortOption,i) in sortOptions" :key="i">
										<q-item
											clickable
											v-close-popup
										...
										>
											<q-item-section>
												<div>
													<q-icon :name="formInput.sortOption.text == sortOption.text ? 'radio_button_checked' : 'radio_button_unchecked'"></q-icon>
													<span class="pl-3">{{ sortOption.text }}</span>
												</div>
											</q-item-section>
										</q-item>
									</template>
					...
					<template v-else>
						<div class="px-8 py-16 text-center">
							...
							<div class="font-bold italic pt-2">No Data Available</div>
						</div>
					</template>
				</div>
			</div>
					...
					<div class="row">
						...
						>
							<q-btn
								...
								unelevated
								:color="filterForm.listing_type == 'buy' ? 'primary' : 'white text-black' "
								label="Buy"
							...
							<q-btn
								...
								unelevated
								:color="filterForm.listing_type == 'rent' ? 'primary' : 'white text-black' "
								label="Rent"
								class="text-capitalize fill-width"
						...
						<div class="row q-gutter-y-md">
								...
								<div class="row q-gutter-y-sm">
									...
									<div class="col col-12 q-gutter-x-sm">
										<template v-for="propertyTypeFilterOption in propertyTypeFilterOptions" :key="propertyTypeFilterOption.id">
											<q-checkbox
												v-model="filterForm.property_types"
												:label="propertyTypeFilterOption.name"
												:val="propertyTypeFilterOption.id"
											/>
										</template>
									</div>
								</div>
										...
										<div class="mx-0 row q-gutter-sm">
											<div class="col">
												...
												<q-input
													...
													placeholder="Min" dense outlined
													@update:model-value="(value) => formatNumberInput('min_price', value, 2)"

												>
													<template #prepend>
														<div class="text-body2">RM</div>
													</template>
												</q-input>
											</div>
											<div class="col">
												...
												<q-input
													...
													placeholder="Max" dense outlined
													@update:model-value="(value) => formatNumberInput('max_price', value, 2)"
												>
													<template #prepend>
														<div class="text-body2">RM</div>
													</template>
									...
									<div class="col col-12">
										<div class="mx-0 row q-gutter-sm">
											<div class="col">
												<div class="pb-1">Minimum</div>
												<q-input
													v-model="filterForm.min_floor_size"
													placeholder="Min" dense outlined
													@update:model-value="(value) => formatNumberInput('min_floor_size', value)"
												>
													<template #append>
														<div class="text-body2">sqft</div>
													</template>
												</q-input>
											</div>
											<div class="col">
												<div class="pb-1">Maximum</div>
												<q-input
													v-model="filterForm.max_floor_size"
													placeholder="Max" dense outlined
													@update:model-value="(value) => formatNumberInput('max_floor_size', value)"
												>
													<template #append>
														<div class="text-body2">sqft</div>
													</template>
							...
							<div class="col col-12">
								<div class="row q-gutter-y-sm">
									...
									<div class="col col-12 q-gutter-x-sm">
										<template v-for="tenureFilterOption in tenureFilterOptions" :key="tenureFilterOption.value">
											<q-checkbox
	...
	methods:{
		...
		formatNumberInput(model, value, decimalPlaces = 0) {
			...
			if(value[value.length - 1] === '.'){
				...
				this.filterForm[model] = integerPart + (decimalPart ? '.' + decimalPart : '');
			}
		}
	}
};
